...
public class EditPatientActivity extends AppCompatActivity {
    ...
    private int originalPatientId; // Сохраняем оригинальный patient_id
    ...
    private void savePatient() {
        ...
        values.put("patient_id", originalPatientId); // Используем оригинальный patient_id
        ...
        values.put("admission_count", patient.getAdmissionCount() + 1); // Увеличиваем количество поступлений

        try {
            long newRowId = db.insert("Patients", null, values);
            if (newRowId != -1) {
                ContentValues historyValues = new ContentValues();
                historyValues.put("patient_id", originalPatientId); // Используем оригинальный patient_id
